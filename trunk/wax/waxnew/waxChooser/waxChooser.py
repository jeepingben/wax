...
class waxGUI(object):
	...
	def build_gui (self):
		...
		gui_items['mainwin'].callback_delete_request_add (destroy)
	...
	#	print str( rhtoolbar.icon_size )
		...
		gui_items['waxicon'].scale_set(0.2)
		gui_items['waxicon'].size_hint_align_set(0.5, 0.0)
		gui_items['mainwin'].resize_object_add (gui_items['waxicon'])
		...
		ut.text_set('Units')
		ut.style_set("toggle")
		ut.part_text_set( 'off', 'Fahrenheit' )
		ut.part_text_set( 'on', 'Celsius' )
		...
		for brand in self.brands:
			...
			ut.style_set("toggle")
			ut.text_set(brand)
			ut.part_text_set( 'on', 'enabled' )
			ut.part_text_set( 'off', 'disabled' )   
			ut.size_hint_weight_set (evas.EVAS_HINT_EXPAND, 0.0)
			ut.size_hint_align_set (evas.EVAS_HINT_FILL, 0.0)
		...
		ut.style_set("toggle")
		ut.part_text_set( 'on', 'Yes' )
		ut.part_text_set( 'off', 'No' )  
		ut.size_hint_weight_set (evas.EVAS_HINT_EXPAND, 0.0)
		ut.size_hint_align_set (evas.EVAS_HINT_FILL, 0.0)
		tb.pack_end( ut )
	...
	def displayWax( self, *args, **kargs ): 
		...
		if bestResult is not None:
			...
			self.widgets['waxicon'].resizable_set(0,0)
